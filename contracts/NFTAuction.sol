...
/**
 * @notice Primary sale auction contract for NonFiggybles NFTs
 */
contract NFTAuction is Context, ReentrancyGuard {
    ...
    constructor(
        ...
        require(address(_NonFiggyblesNft) != address(0), "NFTAuction: Invalid NFT");
    ...
    function resultAuction(uint256 _tokenId) external nonReentrant {
        ...
        require(NonFiggyblesNft.isApproved(_tokenId, address(this)), "NFTAuction.resultAuction: auction not approved");
        ...
        NonFiggyblesNft.setPrimarySalePrice(_tokenId, winningBid);
        ...
        (bool creatorTransferSuccess,) = NonFiggyblesNft.ownerOf(_tokenId).call{value : winningBid.sub(platformFeeInETH)}("");
        require(creatorTransferSuccess, "NFTAuction.resultAuction: Failed to send the designer their royalties");

        // Transfer the token to the winner
        NonFiggyblesNft.safeTransferFrom(NonFiggyblesNft.ownerOf(_tokenId), winner, _tokenId);
